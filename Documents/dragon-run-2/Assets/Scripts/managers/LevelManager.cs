...
public class LevelManager : MonoBehaviour
{
    ...
    private List<Segment> LoadSegmentsForLevel(int levelIndex)
    {
        Debug.Log("[LevelManager] Loading level " + levelIndex);

        var segmentObjects = Resources.LoadAll($"segments/level{levelIndex}/", typeof(GameObject));
        var segments = new List<Segment>(segmentObjects.Length);

        for (int i = 0; i < segmentObjects.Length; i++)
        {
            var gameObject = segmentObjects[i] as GameObject;
            Debug.Log($"[LevelManager] Load {gameObject.name} segment");
            ...
            segments.Add(gameObject.GetComponent<Segment>());
        }

        Debug.Log($"[LevelManager] Loaded {segments.Count} segments");
    ...
    private void SpawnSegment()
    {
        ...
        segment.transform.localPosition = Vector3.forward * (currentSpawnZ + initSegmentOffset);
